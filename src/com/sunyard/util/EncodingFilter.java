 ...
 * @version $Revision: 1.1 $ $Date: 2013/07/03 23:35:32 $
 */
...
public class EncodingFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		// 取消spring mvc编码过滤器，此过滤由spring过滤器处理
		if(uri.indexOf("/app/") != -1) {
			chain.doFilter(request, response);
			return;
		}
		
		String oriencode = request.getCharacterEncoding();
		if(null!=oriencode&&oriencode.equals("UTF-8")){
			chain.doFilter(request, response);
			return;
		}
		if (ignore || (oriencode == null)) {
