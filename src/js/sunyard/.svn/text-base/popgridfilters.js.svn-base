...
//        this.updateColumnHeadings();
    ...
    getQueryWin:function(){
    	...
    	//h =  this.height || h >300 ? h : 300;
    	//alert(this.height+"-"+h+"-"+this.grid.getHeight()/2);
    	var p = this.grid.getPosition();
    	this.win.setSize(w,this.height);    	
...
Ext.ux.sunyard.filter.GridSortFilters = Ext.extend(Ext.util.Observable, {
	encode : true,
	paramPrefix : 'filter',
	type:'sort',
	width:500,
...
//        this.deferredUpdate.cancel();
    ...
    onBeforeLoad : function (store, options) {
        ...
        this.cleanParams(options.params);        
        var params = this.buildQuery(this.getFilterData());
        params['orderbyJson'] = Ext.util.JSON.encode(this.getSortData());
        Ext.apply(options.params, params);
    },
    onLoad : function (store, options) {
        store.filterBy(this.getRecordFilter());
...
//    	this.form.doLayout();
    ...
    destroy : function () {
    	...
    	if(this.sortWin){
    		this.sortWin.destroy();
    		delete this.sortWin;
    		delete this.sortForm;
    	}
    },
    createForm:function(){
    	this.form.removeAll();
            ...
            for (i = 0, len = filters.length; i < len; i++) {
                ...
                if (cm) {
                    ...
                    if (filter){
                        ...
                        //     type specified in store's field's type config
                        filter.type = filter.type || this.store.fields.get(dI).type.type;
                        this.filterFields.push([dI,filters[i].header]);
                    }
                } else {
                    filter = filters[i];
                }
...
//            this.createwin();
    ...
    getSortWin:function(){
    	if(!this.sortForm){
    		//alert(JSON.stringify(this.filterFields));
    		//alert(this.filterFields);
    		this.sortForm = Ext.create({
				xtype:'sunyardform',
				labelAlign:'right',
				autoScroll:true,
				frame:true,
		        border:false,
	       		...
	       		items:[{
					...
					items: [{
						...
						anchor: '95%',
						scope: this,
						handler: this.delSortField
					}]
				}]
			});
    	}
    	if(!this.sortWin){
    		this.sortWin = Ext.create({
				xtype:'window',
				title:'排序设置',
				renderTo:this.grid.container,
				draggable :false,
				...
				buttons:[{
					text:'确定',
					handler:this.onSortSubmit,
					scope:this
				},{
					text:'重置',
					handler:this.onSortReset,
					scope:this
				}]
			});
    	}
        ...
        w = w > this.width ? this.width : w;
    	var p = this.grid.getPosition();
    	this.sortWin.setSize(w,this.height);    	
    	this.sortWin.setPagePosition(p[0]+((this.grid.getWidth())-w)/2,p[1]);
    	return this.sortWin;
    },
    addSortField : function(){
    	var sortfiled=Ext.getCmp('sortfield'+this.sortid).getValue();
		if(sortfiled==null||sortfiled==''){
			MSG.error("请先设置已有排序字段！");
			return;
		}
		else{
			...
			this.sortForm.add({
				xtype: 'compositefield',
				fieldLabel: '排序字段'+this.sortid,
				itemId : 'item'+this.sortid,
				id:'item'+this.sortid,
				items: [{
					xtype:"s_combo",
					flex: 12,
					hideLabel:true,
					id:"sortfield"+this.sortid,
				    combodata:this.filterFields,
					editable: false,
					anchor : '95%'
				},{
			...
			this.sortForm.doLayout();
		}
    },
    delSortField : function(){
		if(this.sortid > 1){
			this.sortForm.remove(Ext.getCmp('item'+this.sortid));
			this.sortid=this.sortid-1;
		}
    ...
    getSortData:function(){
    	var sortData = [];
    	if(this.sortForm){
	    	var formitems = this.sortForm.findByType('s_combo');
	   		for(var i = 1; i <= this.sortid; i++){
	   			var field = Ext.getCmp('sortfield'+i).getValue();
	   			var value = Ext.getCmp('order'+i).getValue();
	   			if(value != ''){
	   				sortData.push({
	                    text:field,
	                    value: value
    ...
    onSortSubmit:function(){
    	if(this.sortForm.getForm().isValid()){ 
    		this.reload();
    		this.sortWin.collapse();
    	}
    },
    onSortReset:function(){
    	this.sortForm.getForm().reset();
    },
    showSortWin:function(){
    	this.getSortWin().show();
    }
});
