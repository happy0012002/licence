...
Ext.ux.sunyard.ExportPlugin = Ext.extend(Ext.util.Observable,{
	...
	onAfterSelectExportField:function(form,values){
		...
		//alert(JSON.stringify(this.exportParam));
		var fd = this.getExportForm(this.exportParam);
	    fd.dom.submit();
	    this._getExportWin().hide();
	},
	_onPrint:function(){
	...
	_exportCurrPage:function(){
		this.exportParam={};
		var filter = this.grid.filtersPlugin;
		if(filter != null){
			var params = filter.buildQuery(filter.getFilterData());
			if(filter.type == 'sort'){
				params['orderbyJson'] = Ext.util.JSON.encode(filter.getSortData());
			}
			Ext.apply(this.exportParam,params);
		}
