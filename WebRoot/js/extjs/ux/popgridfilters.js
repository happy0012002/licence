...
//            this.deferredUpdate.cancel();
            if (this.toolbar) {
                ...
                //alert(Ext.encode(store.paramNames));
    ...
    onBeforeLoad : function (store, options) {
        ...
        //alert("params"+Ext.encode(params));
        Ext.apply(options.params, params);
        //alert("options.params"+Ext.encode(options.params));
//        this.win.collapse();
    },
...
//        this.win.collapse();
        ...
        if (!this.encode){
            ...
            */
            ...
            for (i = 0; i < len; i++) {
            	f = filters[i];
            	//alert(Ext.encode(f));
            	if(f.data.type == 'numeric' || f.data.type == 'date') {
            		tmp += (f.field + '#' + f.data.comparison + '@' + f.data.value + '@');
            	} else {
    ...
    getFilterData : function () {
        ...
        this.filters.each(function (f) {
        	//alert('active:'+f.active+',serialize'+Ext.encode(f.serialize()));
            if (f.active) {
...
Ext.ux.sunyard.filter.Filter = Ext.extend(Ext.util.Observable, {
	...
	constructor : function (config) {
        ...
        Ext.ux.sunyard.filter.Filter.superclass.constructor.call(this);
...
Ext.ux.sunyard.filter.StringFilter = Ext.extend(Ext.ux.sunyard.filter.Filter,{
    ...
    isActivatable : function () {
    	//alert("isActivatable:"+this.getValue());
