Ext.tree.TbarTree = Ext.extend(Ext.tree.TreePanel,{
	...
	initComponent : function(){
		...
		Ext.tree.TbarTree.superclass.initComponent.call(this);
...
Ext.form.ComboTree = Ext.extend(Ext.form.TriggerField,{
	editable: false,
	maxHeight : 300,
	minHeight : 90,
	minChars : 4,
    ...
    initValue : function(){
        Ext.form.ComboTree.superclass.initValue.call(this);
        if(this.hiddenField){
    ...
    initTree:function(){
    	this.innerTree = new Ext.tree.TbarTree({
    ...
    setValue : function(v){
    	...
    	var node = null;
    	if(this.innerTree) {
    		node = this.innerTree.getNodeById(v);
    	}
        if(node){
            text = node.text;
        ...
        Ext.form.ComboTree.superclass.setValue.call(this, text);
        this.value = v;
        return this;
    },
    onDestroy : function(){
    ...
    fireKey : function(e){
        if (!this.isExpanded()) {
            Ext.form.ComboTree.superclass.fireKey.call(this, e);
        }
    },

    // private
    onResize : function(w, h){
        Ext.form.ComboTree.superclass.onResize.apply(this, arguments);
    ...
    // private
    onEnable : function(){
        Ext.form.ComboTree.superclass.onEnable.apply(this, arguments);
        if(this.hiddenField){
    ...
    // private
    onDisable : function(){
        Ext.form.ComboTree.superclass.onDisable.apply(this, arguments);
        if(this.hiddenField){
...
//                this.onLoad();
    ...
    destroy:function(){
    	...
    	Ext.form.ComboTree.superclass.destroy.call(this);
    }
});

Ext.form.TreeField = Ext.extend(Ext.form.Field,{
	valueSeparator:',',
	...
	afterRender : function(){
        Ext.form.TreeField.superclass.afterRender.call(this);
    },
	initTree:function(){
		if(!this.innerTree)
    	this.innerTree = new Ext.tree.TbarTree({
	...
	getErrors: function(value) {
        var errors = Ext.form.TreeField.superclass.getErrors.apply(this, arguments);        
    ...
    destroy:function(){
    	if(this.innerTree) this.innerTree.destroy();
    	Ext.form.TreeField.superclass.destroy.call(this);
    }
});
Ext.tree.AccordionTree = Ext.extend(Ext.Panel,{
	dataUrl:null,
	rootNode:'root',
	layout:'accordion',
	afterRender : function(){
		Ext.tree.AccordionTree.superclass.afterRender.call(this);
	...
	createInnerTree:function(item){
		var _tree = new Ext.tree.TbarTree({
			title:item.text,
            autoScroll:true,           
		    rootVisible:false,
		    lines:true,
...
Ext.reg('s_combotree',Ext.form.ComboTree);
Ext.reg('s_tbartree',Ext.tree.TbarTree);
Ext.reg('s_treefield',Ext.form.TreeField);
Ext.reg('s_accordiontree',Ext.tree.AccordionTree);